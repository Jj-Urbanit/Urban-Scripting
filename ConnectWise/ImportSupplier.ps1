###############################################################################################
################## This script imports supplier data from cw-suppliers.csv  ###################
################## into the missing link site column "Supplier".            ###################
################## It requires the cw-suppliers.csv and                      ##################
...
$password = "cxo1lTZDIE8K7n57!" 
$securePassword = ConvertTo-SecureString $password -AsPlainText -Force 
$logpath = "D:\CW-Export-logs\Import-Suppliers.log"
...
{
	####### CHANGE PATH BELOW ###############
	$importCSV = "D:\CW-Export\cw-suppliers.csv" 
	$FileExists = Test-Path $importCSV 
    if ($FileExists -eq $True)
    {if((Get-Item $importCSV).length -gt 0kb)
	{
		$exportedSupplier = Import-CSV $importCSV 
								...
								if ($contentTypes[$i].Fields[$j].id -eq $guid)
       							{
	  								...
	  								$ctx.Load($contentTypes[$i].Fields[$j])
									$ctx.ExecuteQuery()
	   								$contentTypes[$i].Fields[$j].Choices = $exportedSupplier.Company_Name
...
catch
{
	...
	$Time = Get-Date
	####### CHANGE PATH BELOW ###############
 	"$Time - An error occurred while importing supplier data: $ErrorMessage" | out-file $logpath -Append
}