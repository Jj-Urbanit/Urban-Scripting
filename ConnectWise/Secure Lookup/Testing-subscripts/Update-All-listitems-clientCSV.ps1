# Update_All_listitems_clientCSV.ps1
#
###################################
# Used to update all list items in all client sites from a CSV with their respective client name from Lookup Field
###################################
...
{
    $url = $row.url
    $Output | where {$_.client -eq $row.title} | % {
            $curid = $_.LineID
