<#
    ...
    Creates a HTML Report describing the Exchange environment 
   	...
   	Modified by Marco Liewerenz and Chris Turner
	
	THIS CODE IS MADE AVAILABLE AS IS, WITHOUT WARRANTY OF ANY KIND. THE ENTIRE 
	RISK OF THE USE OR THE RESULTS FROM THE USE OF THIS CODE REMAINS WITH THE USER.
	
	Version 1.6.2 January 2017 / Marco Liewerenz 1.6.3 September 2017
...
function _GetDB
{
    ...
    if ($Database.Mounted) { $IsMounted="Yes" } else { $IsMounted = "No" }
...
# Sub-Function to Get Mailbox Information
function _GetMBInfo
{
	# Mailbox Average Sizes
	$MailboxStatistics = [array](Get-ExchangeServer | Get-MailboxStatistics -WarningAction SilentlyContinue | Select DisplayName,ServerName,Database,TotalItemSize,ItemCount | Sort TotalItemSize -Descending | Select -First 20)
	if ($MailboxStatistics)
	{
		$Output="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif""><tr align=""center"" bgcolor=""#FFD700"">"
		$Output+="<th>Mailbox Name</th><th>ServerName</th><th>Database</th><th>Size</th><th>Items</th>"
		$Output+="</tr>"
		foreach ($mailboxinfo in $MailboxStatistics)
		{
			$Output+="<tr"
			if ($AlternateRow)
			{
			...
			$Output+="><td>$($mailboxinfo.DisplayName)</td>"
			$Output+="<td>$($mailboxinfo.ServerName)</td>"
			$Output+="<td>$($mailboxinfo.Database)</td>"
			$Output+="<td>$($mailboxinfo.TotalItemSize)</td>"
			$Output+="<td>$($mailboxinfo.ItemCount)</td>"
			$Output+="</tr>";
		}
		$Output+="</table><br />"
		$Output
...
{
	# Mailbox Average Sizes
	$CertStatistics = [array](Get-ExchangeCertificate | Select CertificateDomains,Issuer,NotAfter,IsSelfSigned,Status,Services)
	if ($CertStatistics)
	{
		$Output="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif""><tr align=""center"" bgcolor=""#FFD700"">"
		$Output+="<th>Certificate Domain</th><th>Issuer</th><th>Expiry date</th><th>SelftSigned?</th><th>Status</th><th>Services</th>"
		$Output+="</tr>"
		foreach ($certificate in $CertStatistics)
		{
			$Output+="<tr"
			if ($AlternateRow)
			{
			...
			$Output+="><td>$($certificate.CertificateDomains)</td>"
			$Output+="<td>$($certificate.Issuer)</td>"
			$Output+="<td>$($certificate.NotAfter)</td>"
			$Output+="<td>$($certificate.IsSelfSigned)</td>"
			$Output+="<td>$($certificate.Status)</td>"
			$Output+="<td>$($certificate.Services)</td>"
			$Output+="</tr>";
		}
		$Output+="</table><br />"
		$Output
...
function _GetExSvr
{
	...
	if ($ExchangeMajorVersion -ge 8)
	{
		...
		if ($Roles -contains "Mailbox")
		{
			...
			$MailboxStatistics = Get-MailboxStatistics -Server $ExchangeServer -WarningAction SilentlyContinue | Select DisplayName,@{Name="TotalItemSizeB";Expression={$_.TotalItemSize.Value.ToBytes()}},@{Name="TotalDeletedItemSizeB";Expression={$_.TotalDeletedItemSize.Value.ToBytes()}},Database
	    }
...
function _GetOverview
{
	...
	# Creating VirtualDirectory overview
	$Output="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif"">
    	  <tr bgcolor=""#b042f4"" ><th><font color=""#ffffff"">Virtual Directories</font></th></table>"
	
	$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif""><tr align=""center"" bgcolor=""#FFD700"">"
	$Output+="<th>Server</th><th>Type</th><th>InternalURL</th><th>ExternalURL</th><th>InternalAuth</th><th>ExternalAuth</th>"
	$Output+="</tr>"
	
	foreach ($Server in $Servers.Value)
	{
	$VirDirOWAStatistics = [array](Get-OWAVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)
	$VirDirWebStatistics = [array](Get-WebServicesVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)
	$VirDirOABStatistics = [array](Get-OABVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)
	$VirDirASStatistics = [array](Get-ActiveSyncVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)
	$VirDirMAPIStatistics = [array](Get-MAPIVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)
	$VirDirADStatistics = [array](Get-AutodiscoverVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)
	$VirDirECPStatistics = [array](Get-ECPVirtualDirectory -Server $Server.Name | Select Server,InternalURL,ExternalURL,InternalAuthenticationMethods,ExternalAuthenticationMethods)

		
	
		if ($VirDirOWAStatistics)
		{
			foreach ($OWAVirDir in $VirDirOWAStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($OWAVirDir.Server)</td>"
				$Output+="<td>OWA</td>"
				$Output+="<td>$($OWAVirDir.InternalURL)</td>"
				$Output+="<td>$($OWAVirDir.ExternalURL)</td>"
				$Output+="<td>$($OWAVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($OWAVirDir.ExternalAuthenticationMethods)</td>"
			}
		} 
		
		if ($VirDirWebStatistics)
		{
			foreach ($WebVirDir in $VirDirWebStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($WebVirDir.Server)</td>"
				$Output+="<td>WebServices</td>"
				$Output+="<td>$($WebVirDir.InternalURL)</td>"
				$Output+="<td>$($WebVirDir.ExternalURL)</td>"
				$Output+="<td>$($WebVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($WebVirDir.ExternalAuthenticationMethods)</td>"
			}
		}
		
		if ($VirDirOABStatistics)
		{
			foreach ($OABVirDir in $VirDirOABStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($OABVirDir.Server)</td>"
				$Output+="<td>OAB</td>"
				$Output+="<td>$($OABVirDir.InternalURL)</td>"
				$Output+="<td>$($OABVirDir.ExternalURL)</td>"
				$Output+="<td>$($OABVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($OABVirDir.ExternalAuthenticationMethods)</td>"
			}
		}
		
		if ($VirDirASStatistics)
		{
			foreach ($ASVirDir in $VirDirASStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($ASVirDir.Server)</td>"
				$Output+="<td>ActiveSync</td>"
				$Output+="<td>$($ASVirDir.InternalURL)</td>"
				$Output+="<td>$($ASVirDir.ExternalURL)</td>"
				$Output+="<td>$($ASVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($ASVirDir.ExternalAuthenticationMethods)</td>"
			}
		}
		
		if ($VirDirMAPIStatistics)
		{
			foreach ($MAPIVirDir in $VirDirMAPIStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($MAPIVirDir.Server)</td>"
				$Output+="<td>MAPI</td>"
				$Output+="<td>$($MAPIVirDir.InternalURL)</td>"
				$Output+="<td>$($MAPIVirDir.ExternalURL)</td>"
				$Output+="<td>$($MAPIVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($MAPIVirDir.ExternalAuthenticationMethods)</td>"
			}
		}
		
		if ($VirDirADStatistics)
		{
			foreach ($ADVirDir in $VirDirADStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($ADVirDir.Server)</td>"
				$Output+="<td>Autodiscover</td>"
				$Output+="<td>$($ADVirDir.InternalURL)</td>"
				$Output+="<td>$($ADVirDir.ExternalURL)</td>"
				$Output+="<td>$($ADVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($ADVirDir.ExternalAuthenticationMethods)</td>"
			}
		}
		
		if ($VirDirECPStatistics)
		{
			foreach ($ECPVirDir in $VirDirECPStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($ECPVirDir.Server)</td>"
				$Output+="<td>ECP</td>"
				$Output+="<td>$($ECPVirDir.InternalURL)</td>"
				$Output+="<td>$($ECPVirDir.ExternalURL)</td>"
				$Output+="<td>$($ECPVirDir.InternalAuthenticationMethods)</td>"
				$Output+="<td>$($ECPVirDir.ExternalAuthenticationMethods)</td>"
			}
		}
	...
	# Creating Send connector overview
	$Output="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif"">
    	  <tr bgcolor=""#33960f"" ><th><font color=""#ffffff"">Send Connectors</font></th></table>"
	
	$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif""><tr align=""center"" bgcolor=""#FFD700"">"
	$Output+="<th>Server</th><th>Name</th><th>Port</th><th>Fqdn</th><th>AddressSpaces</th><th>DNSRouting</th><th>Enabled</th>"
	$Output+="</tr>"
	

	$SendConnectorStatistics = [array](Get-Sendconnector | Select HomeMtaServerId,Name,Port,Fqdn,AddressSpaces,DNSRoutingEnabled)
	
	if ($SendConnectorStatistics)
	{
			foreach ($SendConnector in $SendConnectorStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($SendConnector.HomeMtaServerId)</td>"
				$Output+="<td>$($SendConnector.Name)</td>"
				$Output+="<td>$($SendConnector.Port)</td>"
				$Output+="<td>$($SendConnector.Fqdn)</td>"
				$Output+="<td>$($SendConnector.AddressSpaces)</td>"
				$Output+="<td>$($SendConnector.DNSRoutingEnabled)</td>"
				$Output+="<td>$($SendConnector.Enabled)</td>"
	...
	# Creating receive connector overview
	$Output="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif"">
    	  <tr bgcolor=""#33960f"" ><th><font color=""#ffffff"">Receive Connectors</font></th></table>"
	
	$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif""><tr align=""center"" bgcolor=""#FFD700"">"
	$Output+="<th>Server</th><th>Name</th><th>Bindings</th><th>Remote IPs</th><th>Permission Groups</th>"
	$Output+="</tr>"
	
	foreach ($Server in $Servers.Value)
	{
	$ReceiveConnectorStatistics = [array](Get-ReceiveConnector -Server $Server.Name | Select Server,Name,Bindings,RemoteIPRanges,PermissionGroups)
	
	if ($ReceiveConnectorStatistics)
	{
			foreach ($ReceiveConnector in $ReceiveConnectorStatistics)
			{
				$Output+="<tr"
				if ($AlternateRow)
				{
				...
				$Output+="><td>$($ReceiveConnector.Server)</td>"
				$Output+="<td>$($ReceiveConnector.Name)</td>"
				$Output+="<td>$($ReceiveConnector.Bindings)</td>"
				$Output+="<td>$($ReceiveConnector.RemoteIPRanges)</td>"
				$Output+="<td>$($ReceiveConnector.PermissionGroups)</td>"
			}
	}
	}
	$Output+="</tr></table><br />"
...
function _GetDBTable
{
	...
	foreach ($Database in $Databases)
	{
		...
		if ($ShowIsMounted)
		{
			$Output+="<td align=""center"">$($Database.IsMounted)</td>";
		}
...
{
	_UpProg1 70 "Writing HTML Non-DAG Database Information" 4
	$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif"">
    	  <tr bgcolor=""#FF8000""><th>Mailbox Databases (Non-DAG)</th></table>"
	$Output+=_GetDBTable -Databases $ExchangeEnvironment.NonDAGDatabases
}

# Reading Top 20 Mailbox Statistics
_UpProg1 80 "Writing HTML Top 20 Mailboxes by size Information" 4
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif"">
    	  <tr bgcolor=""#900C3F"" ><th><font color=""#ffffff"">Top 20 Mailboxes (by size)</font></th></table>"
$Output+= _GetMBInfo

# Reading Certificate Information
_UpProg1 85 "Writing HTML Certificate Information" 4
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Segoe UI,Arial,sans-serif"">
    	  <tr bgcolor=""#0c8490"" ><th><font color=""#ffffff"">Certificates</font></th></table>"
$Output+= _GetCertInfo
